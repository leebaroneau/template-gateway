...
function sanitizeAuditMetadata(metadata: Record<string, string> | undefined): Record<string, string> | undefined {
  ...
  const sanitized = Object.fromEntries(
    Object.entries(metadata).filter(([key, value]) => {
      ...
      return !normalizedKey.includes("secret") && !value.includes("gw_live_");
    })
  );
  ...
  createClient(input: CreateApiClientInput, actor: string): ApiClient {
    ...
    const create = this.db.transaction(() => {
      ...
      this.insertAudit({
        ...
        actor,
        metadata: { owner: input.owner, type: input.type }
      });
    ...
    try {
      ...
      const authenticate = this.db.transaction(() => {
        const rows = this.db
          ...
          client_id: string;
          client_name: string;
          client_type: ApiClient["type"];
          client_status: ApiClient["status"];
          client_owner: string;
        ...
        const matched = rows.find(
          (row) =>
            ...
            verifyApiKeySecret(secret, row.key_secret_hash)
        );
        if (!matched) {
          return undefined;
        }
